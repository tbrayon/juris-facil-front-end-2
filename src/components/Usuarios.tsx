...
export function UsuariosView({ onVoltar }: UsuariosViewProps) {
  ...
  const validarFormulario = (): boolean => {
    ...
    if (formData.senha.trim() && formData.senha !== formData.confirmarSenha) novosErros.confirmarSenha = 'As senhas não coincidem';
                    ...
                    >
                      <div className="flex-1 w-full">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-4 gap-y-1 text-sm text-[#6b5544]">
                          ...
                          <p className="truncate">Usuário: @{usuario.usuario}</p>
                          {usuario.oab && <p className="truncate">OAB: {usuario.oab}</p>}
                        </div>
                      </div>
                      <div className="flex items-center gap-1 sm:gap-2 self-end sm:self-auto mt-4 sm:mt-0">
                        <Button size="icon" variant="ghost" onClick={() => handleVisualizar(usuario)} className="text-[#a16535] hover:text-[#8b5329] hover:bg-[#a16535]/10" title="Visualizar">
                          <Eye className="w-4 h-4" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => handleEditar(usuario)} className="text-[#a16535] hover:text-[#8b5329] hover:bg-[#a16535]/10" title="Editar">
  ...
  if (modoExibicao === 'visualizacao' && usuarioSelecionado) {
    return (
      <div className="max-w-4xl mx-auto space-y-6 p-4">
        <Card className="bg-white border-[#d4c4b0]">
          {/* HEADER RESPONSIVO - DETALHES DO CLIENTE */}
        ...
        <CardContent>
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  <Label htmlFor="nome" className="text-[#6b5544]">Nome Completo *</Label>
                  <Input id="nome" value={formData.nome} onChange={(e) => setFormData({ ...formData, nome: e.target.value })} className="bg-[#f6f3ee] border-[#d4c4b0] text-[#2d1f16] placeholder:text-[#6b5544] focus:border-[#a16535] focus:ring-[#a16535]/20" placeholder="Digite o nome completo" />
                  {erros.nome && <p className="text-sm text-red-600 mt-1">{erros.nome}</p>}
                </div>
                <div>
                  <Label htmlFor="email" className="text-[#6b5544]">E-mail *</Label>
                  <Input id="email" type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="bg-[#f6f3ee] border-[#d4c4b0] text-[#2d1f16] placeholder:text-[#6b5544] focus:border-[#a16535] focus:ring-[#a16535]/20" placeholder="email@exemplo.com" />
                  {erros.email && <p className="text-sm text-red-600 mt-1">{erros.email}</p>}
                </div>
                <div>
                  <Label htmlFor="telefone" className="text-[#6b5544]">Telefone</Label>
                  <Input id="telefone" value={formData.telefone} onChange={(e) => setFormData({ ...formData, telefone: formatTelefone(e.target.value) })} className="bg-[#f6f3ee] border-[#d4c4b0] text-[#2d1f16] placeholder:text-[#6b5544] focus:border-[#a16535] focus:ring-[#a16535]/20" placeholder="(11) 98765-4321" />
                </div>
                <div>
                  <Label htmlFor="tipo" className="text-[#6b5544]">Tipo de Usuário *</Label>
                  <select id="tipo" value={formData.tipo} onChange={(e) => setFormData({ ...formData, tipo: e.target.value as any })} className="w-full h-10 px-3 rounded-md border border-[#d4c4b0] bg-[#f6f3ee] text-[#2d1f16] focus:border-[#a16535] focus:ring-2 focus:ring-[#a16535]/20 outline-none transition-colors">
                ...
                {(formData.tipo === 'advogado' || formData.tipo === 'administrador') && (
                  <div>
                    <Label htmlFor="oab" className="text-[#6b5544]">OAB *</Label>
                    <Input id="oab" value={formData.oab} onChange={(e) => setFormData({ ...formData, oab: e.target.value })} className="bg-[#f6f3ee] border-[#d4c4b0] text-[#2d1f16] placeholder:text-[#6b5544] focus:border-[#a16535] focus:ring-[#a16535]/20" placeholder="OAB/SP 123.456" />
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  <Label htmlFor="usuario" className="text-[#6b5544]">Nome de Usuário *</Label>
                  <Input id="usuario" value={formData.usuario} onChange={(e) => setFormData({ ...formData, usuario: e.target.value.toLowerCase().replace(/\s/g, '') })} className="bg-[#f6f3ee] border-[#d4c4b0] text-[#2d1f16] placeholder:text-[#6b5544] focus:border-[#a16535] focus:ring-[#a16535]/20" placeholder="usuario.login" />
                  {erros.usuario && <p className="text-sm text-red-600 mt-1">{erros.usuario}</p>}
                </div>
                <div>
                  <Label htmlFor="senha" className="text-[#6b5544]">Senha *</Label>
                  <Input id="senha" type="password" value={formData.senha} onChange={(e) => setFormData({ ...formData, senha: e.target.value })} className="bg-[#f6f3ee] border-[#d4c4b0] text-[#2d1f16] placeholder:text-[#6b5544] focus:border-[#a16535] focus:ring-[#a16535]/20" placeholder="Mínimo 6 caracteres" />
                  {erros.senha && <p className="text-sm text-red-600 mt-1">{erros.senha}</p>}
                </div>
                <div>
                  <Label htmlFor="confirmarSenha" className="text-[#6b5544]">Confirmar Senha *</Label>
                  <Input id="confirmarSenha" type="password" value={formData.confirmarSenha} onChange={(e) => setFormData({ ...formData, confirmarSenha: e.target.value })} className="bg-[#f6f3ee] border-[#d4c4b0] text-[#2d1f16] placeholder:text-[#6b5544] focus:border-[#a16535] focus:ring-[#a16535]/20" placeholder="Repita a senha" />
            ...
            <div className="space-y-4">
              <Label htmlFor="observacoes" className="text-[#6b5544]">Observações</Label>
              <Textarea id="observacoes" value={formData.observacoes} onChange={(e) => setFormData({ ...formData, observacoes: e.target.value })} className="bg-[#f6f3ee] border-[#d4c4b0] text-[#2d1f16] placeholder:text-[#6b5544] focus:border-[#a16535] focus:ring-[#a16535]/20 min-h-[100px]" placeholder="Informações adicionais sobre o usuário..." />
            </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-4">
              ...
              >
                <X className="w-4 h-4 mr-2 sm:mr-2" />
                Cancelar
              </Button>
              <Button
